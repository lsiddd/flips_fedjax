...
class FedAvgTest(absltest.TestCase):
    ...
    def test_federated_averaging(self):
        ...
        with self.subTest("init"):
            state = algorithm.init({"w": jnp.array([0.0, 2.0, 4.0])})
            npt.assert_array_equal(state.params["w"], [0.0, 2.0, 4.0])
            self.assertLen(state.opt_state, 2)
        with self.subTest("apply"):
            clients = [
                (
                    b"cid0",
                    fedjax.ClientDataset({"x": jnp.array([2.0, 4.0, 6.0])}),
                    jax.random.PRNGKey(0),
                ),
                (
                    b"cid1",
                    fedjax.ClientDataset({"x": jnp.array([8.0, 10.0])}),
                    jax.random.PRNGKey(1),
                ),
            ]
            state, client_diagnostics = algorithm.apply(state, clients)
            npt.assert_allclose(state.params["w"], [0.0, 1.4425802, 2.8851604])
            npt.assert_allclose(client_diagnostics[b"cid0"]["delta_l2_norm"], 1.7553135)
            npt.assert_allclose(
                client_diagnostics[b"cid1"]["delta_l2_norm"], 0.48310122
            )
